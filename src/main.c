...
int main (int argc, char **argv)
{
  ...
  /* Some options are informational by their nature, we just check if user
     wants to see some info and print it if it's the case. */
  if (opHelp)
    {
      printf (LSA_HELP);
      return EXIT_SUCCESS;
  ...
  if (optind >= argc) free (temp);
  /* First of all, we should check if the given directory does exit. */
  struct stat sb;
      ...
      if (a)
        {
          ...
          printf ("%6d %-2d %c %d ",
                  a->rate,
                  a->width,
                  decodeFormat (a->format),
                  a->channels);
          if (showHours) printf ("%02d:", dur_h);
          printf ("%02d:%02d ", dur_m, dur_s);
          if (opFrames) printf ("%10ld ", a->frames);
          if (opCompression)
            printf ("%11s ", decodeCompression (a->compression));
          printf ("%s\n", a->name);
          free (a);
        }
    }
